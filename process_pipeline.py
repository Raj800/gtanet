...
if __name__ == '__main__':
    ...
    if mode == 'video':
        last_time = time.time()
        while (True):
            cap = cv2.VideoCapture("project_video_output-1.mp4")
            ...
            '''
            printscreen = np.array(ImageGrab.grab(bbox=(0, 40, 800, 620)))
            print('loop took {} seconds'.format(time.time() - last_time))
            last_time = time.time()
            cv2.imshow('window', process_pipeline(cv2.cvtColor(printscreen, cv2.COLOR_BGR2RGB)))
